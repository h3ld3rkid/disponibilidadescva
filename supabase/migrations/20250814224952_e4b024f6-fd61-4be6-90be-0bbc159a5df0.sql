-- Reset just our tables and recreate simple structure
-- Skip auth.users modifications

-- Drop our tables only
...
-- Create users table (simple, no RLS)
...
-- Create schedules table (simple, no RLS)
...
-- Create announcements table (simple, no RLS)
...
-- Create other tables without RLS
...
VALUES (
  'Base de Dados Limpa',
  'Sistema recriado do zero sem RLS. Tudo deve funcionar perfeitamente agora.',
  now(),
