...
import ExchangeRequestSplash from '@/components/auth/ExchangeRequestSplash';

const Login = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { session, createSession, isLoading: sessionLoading } = useAuthSession();
    ...
    try {
      ...
      if (response.success && response.user) {
        ...
        if (response.user.needsPasswordChange) {
          ...
          // Show exchange requests splash screen
  ...
  return (
    <>
      ...
      {showExchangeSplash && loggedInUser && (
        <ExchangeRequestSplash
